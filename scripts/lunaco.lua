...
local function printMonthCalendar(year, monthAlpha, monthNumber)
  ...
  for d=1,numDays do
    ...
    if d-1 == today and OPTIONS['--no-format'] then
      currentLine = currentLine .. customTags .. dayToString(d)
    elseif d == today and OPTIONS['--no-format'] then
      currentLine = currentLine .. "<" .. customTags .. dayToString(d) .. ">"
    else
      currentLine = currentLine .. " " .. customTags .. dayToString(d)
    end
...
elseif arg[1] then
  ...
  if not OPTIONS['--no-format'] then print(generateFontTag(), generateColorTag()) else print(" ") end
